...
//        task.resume()
    ...
    override func viewDidLoad() {
        ...
        let musicFile = Bundle.main.path(forResource:"mgs", ofType:".mp3")
...
//        save to database
        client.callFunction(withName: "add_danger", withArgs: [category_chosen, coordinate_to_save?.latitude.description ?? "0", coordinate_to_save?.longitude.description ?? "0", date_to_save ?? "0"], withRequestTimeout: 5.0
        ) { (result: StitchResult<String>) in
            switch result {
            case .success(let stringResult):
                print("String result: \(stringResult)")
